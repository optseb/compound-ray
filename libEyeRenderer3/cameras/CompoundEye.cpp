...
CompoundEye::CompoundEye(const std::string name, const std::string shaderName, size_t ommatidialCount, const std::string& eyeDataPath)
    : DataRecordCamera<CompoundEyeData>(name), shaderName(NAME_PREFIX + shaderName)
{
...
void
CompoundEye::setShaderName(const std::string shaderName)
{
    this->shaderName = NAME_PREFIX + shaderName;
}

void
CompoundEye::setOmmatidia(Ommatidium* ommatidia, size_t count)
{
...
void
CompoundEye::reconfigureOmmatidialCount(size_t count)
{
...
void
CompoundEye::copyOmmatidia(Ommatidium* ommatidia)
{
    std::cout << "Copying data.." << std::endl;
    CUDA_CHECK(cudaMemcpy(reinterpret_cast<void*>(specializedData.d_ommatidialArray), ommatidia, sizeof(Ommatidium) * specializedData.ommatidialCount, cudaMemcpyHostToDevice));
    std::cout << "  ...data copied." << std::endl;
    CUDA_SYNC_CHECK();
}

void
CompoundEye::allocateOmmatidialMemory()
{
...
void
CompoundEye::freeOmmatidialMemory()
{
#ifdef DEBUG
...
void
CompoundEye::allocateOmmatidialRandomStates()
{
...
#endif
    // TODO(RANDOMS): The randomStateBuffer is currently unitialized. For now we'll be initializing it with if statements in the ommatidial shader, but in the future a CUDA function could be called
    // here to initialize it. Set this camera's randomsConfigured switch to false so that the aforementioned if statement can work:
    specializedData.randomsConfigured = false;
    CUDA_SYNC_CHECK();
}
void
CompoundEye::freeOmmatidialRandomStates()
{
#ifdef DEBUG
...
void
CompoundEye::allocateCompoundRenderingBuffer()
{
...
void
CompoundEye::freeCompoundRenderingBuffer()
{
#ifdef DEBUG
...
void
CompoundEye::setSamplesPerOmmatidium(int32_t s)
{
...
void
CompoundEye::changeSamplesPerOmmatidiumBy(int32_t d)
{
...
void
CompoundEye::InitiateCompoundRecord(OptixShaderBindingTable& compoundSbt, OptixProgramGroup& compoundProgramGroup, const CUdeviceptr& targetRecord)
{
...
void
CompoundEye::RedirectCompoundDataPointer(OptixProgramGroup& programGroup, const CUdeviceptr& targetRecord)
{
#ifdef DEBUG
...
#endif
    CUDA_CHECK(cudaMemcpy(reinterpret_cast<void*>(s_d_compoundRecordPtrRecord), &s_compoundRecordPtrRecord, sizeof(s_compoundRecordPtrRecord), cudaMemcpyHostToDevice));
#ifdef DEBUG
