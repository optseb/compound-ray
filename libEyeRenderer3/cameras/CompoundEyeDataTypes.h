...
struct CompoundEyeData {
    ...
    bool randomsConfigured = false;    // Flag to track whether the random state buffer has been configured, as they need to be before being used. Ultimately this will be replaced by explicitly
                                       // configuring randoms on memory set. TODO(RANDOMS)

    inline bool
    operator==(const CompoundEyeData& other)
    {
        return this->ommatidialCount == other.ommatidialCount && this->d_ommatidialArray == other.d_ommatidialArray && this->samplesPerOmmatidium == other.samplesPerOmmatidium &&
               this->d_randomStates == other.d_randomStates && this->d_compoundBuffer == other.d_compoundBuffer &&
