...
template <typename T> class DataRecordCamera : public GenericCamera
{
  public:
    ...
    getPosition() const
    {
        return sbtRecord.data.position;
    }
    void
    ...
    void
    setLocalSpace(const float3 xAxis, const float3 yAxis, const float3 zAxis)
    {
        ls.xAxis = xAxis;
        ls.yAxis = yAxis;
        ls.zAxis = zAxis;
    ...
    void
    lookAt(const float3& pos)
    {
        lookAt(pos, make_float3(0.0f, 1.0f, 0.0f));
    }
    void
    lookAt(const float3& pos, const float3& upVector)
    {
    ...
    void
    moveLocally(const float3& localStep)
    {
        move(transformToLocal(localStep));
    }
    void
    move(const float3& step)
    {
    ...
    bool
    packAndCopyRecordIfChanged(OptixProgramGroup& programGroup)
    {
    ...
    void
    forcePackAndCopyRecord(OptixProgramGroup& programGroup)
    {
#ifdef DEBUG
        std::cout << "Copying device memory for camera '" << getCameraName() << "'." << std::endl;
        printf("\td_record: %p\n", d_record);
        std::cout << "\tproggroup: " << programGroup << std::endl;
        std::cout << "\tPosition: (" << sbtRecord.data.position.x << ", " << sbtRecord.data.position.y << ", " << sbtRecord.data.position.z << ")" << std::endl;
        std::cout << "\tLocalSpace: ((" << sbtRecord.data.localSpace.xAxis.x << ", " << sbtRecord.data.localSpace.xAxis.y << ", " << sbtRecord.data.localSpace.xAxis.z << ")" << std::endl
                  << "\t              " << sbtRecord.data.localSpace.yAxis.x << ", " << sbtRecord.data.localSpace.yAxis.y << ", " << sbtRecord.data.localSpace.yAxis.z << ")" << std::endl
                  << "\t              " << sbtRecord.data.localSpace.zAxis.x << ", " << sbtRecord.data.localSpace.zAxis.y << ", " << sbtRecord.data.localSpace.zAxis.z << ")" << std::endl;
#endif
        OPTIX_CHECK(optixSbtRecordPackHeader(programGroup, &sbtRecord));
        CUDA_CHECK(cudaMemcpy(reinterpret_cast<void*>(d_record), &sbtRecord, sizeof(sbtRecord), cudaMemcpyHostToDevice));
        previous_sbtRecordData = sbtRecord.data;
    }

    virtual const CUdeviceptr&
    getRecordPtr() const
    {
        return d_record;
    }
