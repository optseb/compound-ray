...
template <typename T> struct RaygenRecord {
    __align__(OPTIX_SBT_RECORD_ALIGNMENT) char header[OPTIX_SBT_RECORD_HEADER_SIZE];
    T data;
};

inline bool
operator==(const float3 l, const float3 r)
{
    return (l.x == r.x && l.y == r.y && l.z == r.z);
}
inline bool
operator==(const float2 l, const float2 r)
{
...
template <typename T> struct RaygenPosedContainer {
    T specializedData;
    float3 position = { 0.0f, 0.0f, 0.0f };
    LocalSpace localSpace;

    inline bool
    operator==(const RaygenPosedContainer<T>& r)
    {
...
template <typename T> using RaygenPosedContainerRecord = RaygenRecord<RaygenPosedContainer<T>>;
