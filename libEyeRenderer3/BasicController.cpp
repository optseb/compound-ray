...
bool
BasicController::ingestKeyAction(int32_t key, int32_t action)
{
    bool output = false;
...
float3
BasicController::getMovementVector()
{
...
float
BasicController::getVerticalRotationAngle()
{
    float out = 0.0f;
    if (rotUp) out += angularSpeed;
    if (rotDown) out -= angularSpeed;
    return out;
}

float
BasicController::getHorizontalRotationAngle()
{
    float out = 0.0f;
    if (rotLeft) out += angularSpeed;
    if (rotRight) out -= angularSpeed;
    return out;
}
