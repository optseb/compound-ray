...
// void pointCameraAt
void setCameraLocalSpace(float lxx, float lxy, float lxz, float lyx, float lyy, float lyz, float lzx, float lzy, float lzz);
...
void setCurrentEyeSamplesPerOmmatidium(int s); // Changes the current eye samples per ommatidium. WARNING: This resets the random seed values. A render must be called to regenerate them, this will
                                               // take significantly longer than a frame render.
int getCurrentEyeSamplesPerOmmatidium(void);   // Returns the current eye samples per ommatidium
void changeCurrentEyeSamplesPerOmmatidiumBy(int s); // Changes the current eye samples per ommatidium. WARNING: This resets the random seed values. A render must be called to regenerate them, this
                                                    // will take significantly longer than a frame render.
